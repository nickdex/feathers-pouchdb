...
class Service {
  ...
  _find(params) {
    params.query =
      _.isObject(params.query) && !_.isEmpty(params.query)
        ? params.query
        : this.query;

    if (
      _.isArray(params.query.$select) &&
      _.indexOf(params.query.$select, this.id) < 0
    ) {
      params.query.$select.push(this.id);
    }

    return this.Model.find(convertQuery(params.query)).then(
  ...
  find(params) {
    if (!_.isEmpty(this.paginate)) {
      ...
      if (params.paginate.default) {
        params.query.$limit = computeLimit(
          params.query.$limit,
          params.paginate
        );
    ...
    return this._find(params).then(records => {
  ...
  update(id, data, params) {
    ...
    findParams.query = _.omit(findParams.query, [
      '$select',
      '$sort',
      '$skip',
      '$limit'
